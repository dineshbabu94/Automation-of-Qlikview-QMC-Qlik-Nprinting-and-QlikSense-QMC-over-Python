...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler(os.getenv("LOG_FILE", "alert_logics.log"))
    ]
)
logger = logging.getLogger()

# Hard-code the environment file path
ENV_FILE = r"D:\OPS_Streamline\Nprinting\Variable\Nprinting_Alerts_common.env"
...
REQUIRED_VARS = [
    "NPRINTING_DIR", "AVERAGE_TIME_FILE", "ALERT_HISTORY_FILE", "HISTORY_OUTPUT_DIR",
...
NPRINTING_DIR = os.getenv("NPRINTING_DIR")
...
PARQUET_FILE = os.path.join(NPRINTING_DIR, f"nprinting_executions_{current_date.strftime('%m_%Y')}.parquet")
...
def send_alert(task_name, start_date, status, time_taken, avg_time, start_time, end_time, run_status, task_freq=None):
    ...
    try:
        ...
        if status == 'Running':
            ...
            if status in ['Failed', 'Aborted', 'CompletedWithWarning']:
        ...
        display_status = "Warning" if status == "CompletedWithWarning" else status
        ...
        if not use_new_api:
            if status == 'Running' and alert_counts.get(task_key, 1) >= PRIORITY_REMINDER_THRESHOLD:
                use_new_api = True
            elif status in ['Failed', 'Aborted', 'CompletedWithWarning']:
                use_new_api = True
...
def read_parquet_file(parquet_file):
    ...
    try:
        ...
        if not df.empty:
            ...
            df['start_date'] = pd.to_datetime(df['n_start_datetime']).dt.strftime('%Y-%m-%d')
            ...
            try:
                ...
                status_issue_mask = df['status'].isin(['Failed', 'Aborted', 'CompletedWithWarning'])
                
                df.loc[time_issue_mask | status_issue_mask, 'run_status'] = 'Issue'
                
                status_change_mask = pd.Series(False, index=df.index)
                for idx, row in df.iterrows():
                    if row['status'] in ['Failed', 'Aborted', 'CompletedWithWarning']:
...
def main():
    ...
    while True:
        try:
            ...
            global PARQUET_FILE
            PARQUET_FILE = os.path.join(NPRINTING_DIR, f"nprinting_executions_{current_time.strftime('%m_%Y')}.parquet")
            
            output_file = os.path.join(output_dir, f"{HISTORY_FILE_PREFIX}{current_date.strftime('%Y%m%d')}.parquet")
                ...
                if current_modified is None:
                    ...
                    for attempt in range(PARQUET_READ_RETRIES):
                        ...
                        if df is not None:
                            ...
                            # Filter tasks that started today and have status='Running' or run_status='Issue'
